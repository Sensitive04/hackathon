...
const landingFeatures = [
  {
    ...
    link: "/satellite",
    color: "text-blue-500",
    bgLight: "bg-neu-bg shadow-neu-pressed-sm",
  },
  {
    icon: ClipboardList,
    title: "Activity",
    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-green-900 via-emerald-800 to-teal-900 text-white">
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-36 relative z-10">
          <div className="max-w-4xl animate-fade-in-up">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl px-4 py-1.5 mb-6">
            ...
            <div className="flex flex-wrap gap-4">
              <Link
                to="/register"
                className="inline-flex items-center gap-2.5 bg-white text-green-800 font-bold py-3.5 px-8 rounded-2xl hover:bg-green-50 transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0"
              >
                Get Started Free
              ...
              <Link
                to="/login"
                className="inline-flex items-center gap-2 border-2 border-white/20 text-white font-semibold py-3.5 px-8 rounded-2xl hover:bg-white/10 hover:border-white/30 transition-all duration-300"
              >
                Sign In
              </Link>
            </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-neu-bg to-transparent" />
      </section>

      {/* Stats bar */}
      <section className="relative -mt-8 z-10">
        <div className="max-w-5xl mx-auto px-4">
          <div className="bg-neu-bg rounded-3xl shadow-neu-raised-lg p-6 grid grid-cols-3 gap-6 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            {[
              ...
              <div key={s.label} className="flex items-center gap-3 justify-center">
                <div className="w-10 h-10 bg-neu-bg rounded-full flex items-center justify-center flex-shrink-0 shadow-neu-pressed-sm">
                  <s.icon className="w-5 h-5 text-eco-primary" />
                </div>
                <div>
                  <p className="text-sm font-bold text-neu-text tracking-tight">{s.value}</p>
                  <p className="text-xs text-neu-text-muted">{s.label}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="features" className="py-20">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-neu-text-secondary max-w-3xl mx-auto leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {landingFeatures.map((f, i) => (
              ...
              >
                <div className={`w-12 h-12 ${f.bgLight} rounded-full flex items-center justify-center mb-4`}>
                  <f.icon className={`w-6 h-6 ${f.color}`} />
                </div>
                <h3 className="text-lg font-bold text-neu-text mb-2 group-hover:text-eco-primary transition-colors duration-200 tracking-tight flex items-center gap-2">
                  {f.title}
                  {"protected" in f && f.protected && (
                    <span className="badge bg-neu-amber-light text-amber-700 shadow-neu-pressed-sm !text-[10px]">
                      <Lock className="w-2.5 h-2.5" />
                      Sign in required
                    </span>
                  )}
                </h3>
                <p className="text-neu-text-secondary text-sm leading-relaxed mb-4">{f.desc}</p>
      ...
      <section className="bg-gradient-to-r from-eco-primary via-emerald-500 to-teal-500 text-white py-20 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center px-4 relative z-10">
          ...
          <Link
            to="/register"
            className="inline-flex items-center gap-2.5 bg-white text-green-700 font-bold py-3.5 px-8 rounded-2xl hover:bg-green-50 transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0"
          >
      ...
      {/* Footer */}
      <footer className="bg-neu-text text-gray-400 py-12">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="flex items-center justify-center gap-2.5 mb-4">
            <div className="w-7 h-7 bg-neu-bg rounded-full flex items-center justify-center shadow-neu-raised-sm">
              <Leaf className="w-4 h-4 text-eco-primary" />
            </div>
            <span className="text-white font-bold text-lg tracking-tight">Smart & Green City</span>
          </div>
...
function AuthenticatedDashboard() {
  ...
  return (
    <div className="page-container">
      ...
      <h2 className="text-lg font-bold text-neu-text mb-4 tracking-tight">Quick Actions</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {actions.map((action) => (
          <Link key={action.title} to={action.link} className="card-interactive group">
            <div className="flex items-start gap-4">
              <div className={`w-12 h-12 ${action.bgLight} rounded-full flex items-center justify-center flex-shrink-0`}>
                <action.icon className={`w-6 h-6 ${action.color}`} />
              </div>
              <div>
                <h3 className="font-bold text-neu-text group-hover:text-eco-primary transition-colors duration-200 tracking-tight">
                  {action.title}
                </h3>
                <p className="text-sm text-neu-text-secondary mt-0.5">{action.desc}</p>
              </div>
            </div>
          </Link>
        ))}
